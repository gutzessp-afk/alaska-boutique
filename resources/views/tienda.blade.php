<x-tienda-layout title="Alaska · Tienda">
    ...
    <section id="tienda" class="max-w-7xl mx-auto px-6 mt-24">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-5">
            @php
                $categorias = [
                    ['nombre' => 'Vestidos',   'img' => 'cat-vestidos.png'],
                    ['nombre' => 'Blusas',     'img' => 'cat-blusas.png'],
                    ['nombre' => 'Faldas',     'img' => 'cat-faldas.png'],
                    ['nombre' => 'Accesorios', 'img' => 'cat-accesorios.png'],
                ];
            @endphp

            @foreach ($categorias as $categoria)
                <a href="#" class="group relative aspect-[3/4] rounded-2xl overflow-hidden block">
                    <img src="{{ asset('images/productos/' . $categoria['img']) }}"
                         alt="{{ $categoria['nombre'] }}"
                         class="absolute inset-0 w-full h-full object-cover transition duration-500 group-hover:scale-105">
                    <div class="absolute inset-0 bg-gradient-to-t from-[#1B2A4A]/70 via-transparent to-transparent"></div>
                    <span class="absolute bottom-5 left-5 font-display text-xl text-white">{{ $categoria['nombre'] }}</span>
                </a>
            @endforeach
        </div>
    </section>
